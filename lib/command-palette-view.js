...
export default class CommandPaletteView {
  ...
  copyItemText (propertyName, event) {
    ...
    if (item) {
      const paramName = propertyName === "name" ? "surroundByBacktickWhenCopyCommandName" : "surroundByBacktickWhenCopyDisplayName"
      let textToClip = item[propertyName]
      if (atom.config.get(`command-palette.${paramName}`)) {
        textToClip = "`" + textToClip + "`"
      }
