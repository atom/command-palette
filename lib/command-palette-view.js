...
export default class CommandPaletteView {
  ...
  copyItemText (propertyName, event) {
    const item = event
      ? this.itemForElement(event.target.closest("li")) // by contextMenu
      : this.selectListView.getSelectedItem() // by keyborad shortcut

    if (item) {
      let surroundByBacktick = false

      if (propertyName === "name") {
        surroundByBacktick = atom.config.get('command-palette.surroundByBacktickWhenCopyCommandName')
      } else if (propertyName === "displayName") {
        surroundByBacktick = atom.config.get('command-palette.surroundByBacktickWhenCopyDisplayName')
      }
      const surroundChar = surroundByBacktick ? "`" : ""
      atom.clipboard.write(surroundChar + item[propertyName] + surroundChar)
    }
  }

  itemForElement (element) {
    if (element && this.selectListView.refs.items) {
      const index = Array.from(this.selectListView.refs.items.children).indexOf(element)
      if (index >= 0) {
        return this.selectListView.items[index]
      }
    }
  }
  ...
  registerCommands () {
    ...
    atom.commands.add(queryEditor.element, {
      "core:copy": event => {
        const propertyToCopy = atom.config.get('command-palette.allowCoreCopyWithEmptySelectionCopySelectedItemText')
